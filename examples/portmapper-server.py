...
from sunrpc.server import UDPServer, rpc_server_obtain, rpc_server_return


class PortMapperServer(UDPServer):
    '''
    Example implementation for a portmapper server. Do not use this in production, it is
    just an example :D
    '''
    def __init__(self, host: str, port: str) -> None:
        ...
        '''
        UDPServer.__init__(self, host, port, 100000, 2)
    ...
    @rpc_server_obtain(RpcInt, RpcInt, RpcInt, RpcInt)
    @rpc_server_return(RpcBool)
    ...
    @rpc_server_obtain(RpcInt, RpcInt, RpcInt, RpcInt)
    @rpc_server_return(RpcBool)
    def unset(self, prog: int, vers: int, prot: int, port: int) -> bool:
        '''
    ...
    @rpc_server_obtain(RpcInt, RpcInt, RpcInt, RpcInt)
    @rpc_server_return(RpcInt)
    def get_port(self, prog: int, vers: int, prot: int, port: int) -> bool:
        '''
    ...
    @rpc_server_obtain()
    @rpc_server_return(RpcList(RpcFArray(RpcInt, 4)))
    def dump(self) -> list:
        '''
